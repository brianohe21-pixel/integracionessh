...
import type { TenantMemberRole } from "../../types/index.js";
...
async function createCognitoTenantUser(
  client: CognitoIdentityProviderClient,
  params: {
    email: string;
    name: string;
...
export async function inviteTenantUser(params: {
  email: string;
  name: string;
  tenantId: string;
  role: TenantMemberRole;
  ...
  try {
    return await createCognitoTenantUser(client, payload);
  } catch (error) {
    if (!(error instanceof UsernameExistsException)) {
      throw error;
    }

    if (params.role === "advisor") {
