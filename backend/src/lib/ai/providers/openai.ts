...
import type { BotLocale, Message } from "../../../types/index.js";
...
import { getModelDefinition, resolveModelId } from "../models.js";
import type {
  AiChatProvider,
  ChatCalendarContext,
...
function buildCompletionRequest(params: {
  ...
  const definition = getModelDefinition(params.modelId);
  const request: OpenAI.ChatCompletionCreateParamsNonStreaming = {
    model: params.modelId,
    messages: params.messages,
  };

  if (definition?.usesCompletionTokens) {
  ...
  if (params.tools && params.tools.length > 0 && definition?.supportsTools !== false) {
...
export class OpenAIProvider implements AiChatProvider {
  ...
  async completeJson<T>(params: JsonCompletionParams): Promise<T> {
    ...
    const definition = getModelDefinition(params.modelId);
    const temperature = definition?.supportsTemperature === false
      ? undefined
      : (params.temperature ?? definition?.defaultTemperature ?? 0.3);
  ...
  async generateChatResponse(params: ChatCompletionParams): Promise<ChatCompletionResult> {
    const locale = params.locale ?? "es";
    const { bot, conversationHistory, userMessage, apiKey, tenantId, calendarContext } = params;
    ...
    const temperature = bot.temperature ?? definition?.defaultTemperature ?? 0.7;
    ...
    if (definition?.supportsTools !== false) {
      tools.push({
        type: "function",
        function: {
