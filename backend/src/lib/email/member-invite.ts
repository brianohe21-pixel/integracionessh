...
function inviteEmailFailureReason(error: unknown): NonNullable<MemberInviteEmailResult["failureReason"]> {
...
export async function sendMemberInviteEmail(params: {
  ...
  if (!from) {
    console.warn("SES_FROM_EMAIL is not configured; skipping member invite email");
  ...
  const text = [
    ...
    "",
    `${params.tenantName} te ha creado una ${roleLabel}.`,
    "",
  ...
  const html = `
    <p>Hola ${escapeHtml(params.memberName)},</p>
    <p><strong>${escapeHtml(params.tenantName)}</strong> te ha creado una ${escapeHtml(roleLabel)}.</p>
