    ...
    <div className="bg-surface-elevated rounded-xl border border-default overflow-hidden">
      <div className="px-6 py-4 border-b border-subtle space-y-4">
        ...
        <select
          ...
          className="w-full sm:max-w-md px-3 py-2 border border-default rounded-lg text-sm bg-surface-elevated focus:outline-none focus:ring-2 focus:ring-accent"
        >
      ...
      {!selectedKeyId && (
        <p className="text-sm text-muted text-center py-10">{t("developer.logsPickKey")}</p>
      )}

      {selectedKeyId && isLoading && (
        <div className="p-6 space-y-3">
          {[...Array(4)].map((_, index) => (
            <div key={index} className="h-12 bg-surface-muted rounded-lg animate-pulse" />
          ))}
        </div>
      )}
                ...
                return (
                  <Fragment key={log.logId}>
                    <tr className="align-top">
                      ...
                      <td className="px-4 py-3">
                        <div className="font-mono text-xs text-primary">{log.method}</div>
                        <div className="font-mono text-xs text-secondary">{log.endpoint}</div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right text-secondary">{log.durationMs} ms</td>
                    </tr>
                    {isExpanded && hasDetails && (
                      <tr className="bg-surface">
                        <td colSpan={5} className="px-6 py-4 space-y-3">
                          {log.errorMessage && (
                            <div>
                              <p className="text-xs font-medium uppercase tracking-wide text-secondary mb-1">
                          ...
                          {log.errorStack && (
                            <div>
                              ...
                              <pre
                                className={cn(
                                  "text-xs font-mono text-primary bg-surface-elevated border border-default",
