...
interface MetricsInboxStatusChartProps {
  marketing: MarketingMetrics;
}

export function MetricsInboxStatusChart({ marketing }: MetricsInboxStatusChartProps) {
  const t = useT();
  const { formatNumber } = useFormatters();
  const inbox = marketing.inbox;

  const data = [
    { key: "open", label: t("metrics.inboxOpen"), value: inbox.open },
    { key: "pending", label: t("metrics.inboxPending"), value: inbox.pending },
    { key: "resolved", label: t("metrics.resolvedToday"), value: inbox.resolvedToday },
  ];
