...
interface DashboardConversationCategoriesChartProps {
  categories?: ConversationCategoryMetrics | null;
  isLoading: boolean;
  error?: Error | null;
}

export function DashboardConversationCategoriesChart({
  categories,
  isLoading,
  error,
}: DashboardConversationCategoriesChartProps) {
  const t = useT();
  const { formatNumber } = useFormatters();

  const data = (categories?.byCategory ?? []).map((row) => ({
    key: row.category,
