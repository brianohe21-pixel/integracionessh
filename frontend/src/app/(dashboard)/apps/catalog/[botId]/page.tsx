...
export default function CatalogBotPage() {
  ...
  return (
    <DashboardPage maxWidth="5xl">
      ...
      <div className="mb-6 flex flex-wrap gap-2 border-b border-default pb-2">
      ...
      {tab === "config" ? (
        <div className="space-y-6">
          <section className="rounded-xl border border-default bg-surface-elevated p-6 space-y-4">
            <h2 className="text-base font-semibold text-primary">{t("catalog.metaCatalogTitle")}</h2>
            <p className="text-sm text-secondary">{t("catalog.metaCatalogHint")}</p>
            <div className="flex flex-wrap items-end gap-3">
              <select
                value={selectedCatalogId}
                onChange={(e) => setSelectedCatalogId(e.target.value)}
                className="min-w-[240px] rounded-lg border border-default px-3 py-2 text-sm bg-surface-elevated"
              >
