...
export default function NewMetaFlowPage() {
  ...
  return (
    <DashboardPage maxWidth="3xl">
      ...
      <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4 bg-surface-elevated border border-default rounded-xl p-6">
        <div>
          <label className="block text-sm font-medium text-secondary mb-1">{t("flows.colName")}</label>
          <input
            value={name}
        ...
        <div>
          <label className="block text-sm font-medium text-secondary mb-1">Template</label>
          <select
            value={template}
            onChange={(e) => setTemplate(e.target.value as "lead_capture" | "feedback")}
            className="w-full border border-default rounded-lg px-3 py-2 text-sm bg-surface-elevated"
          >
        ...
        <button
          ...
          className="w-full py-2 text-sm font-medium text-white bg-accent rounded-lg hover:bg-accent-hover disabled:opacity-50"
        >
          {t("metaFlows.new")}
        </button>
      </form>
